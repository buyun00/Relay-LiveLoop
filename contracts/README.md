...
`operations.json` is the machine-readable operation catalog. `provider-contracts.md` describes the dependency direction between the command service and generic Unity providers. `editor-transport.md` defines the durable, idempotent Host-to-Editor request/result boundary. `native-runtime-update-host.md` documents the internal Host integration seam and open Player response dependencies; it does not add public v1 operations.
