...
if ([string]::IsNullOrWhiteSpace($UnityRoot)) {
    ...
    if ($null -ne $candidate) {
        $UnityRoot = $candidate.FullName
    }
}

if ([string]::IsNullOrWhiteSpace($UnityRoot)) {
    throw 'Unity 2022.3 root was not supplied or discovered.'
}
...
try {
    ...
    $testExe = Join-Path $outputRoot 'RelayLiveLoop.SyntheticTests.exe'
    $compileArgs = @(
        '/nologo',
        '/langversion:9.0',
    ...
    ) + $runtimeSources + $editorSources + $testSources
    ...
    $unityCompileDll = Join-Path $outputRoot 'RelayLiveLoop.UnityCompileCheck.dll'
    ...
    Write-Output 'UNITY REFERENCE COMPILE PASS: Runtime/Core and Editor/Core against Unity 2022.3 managed assemblies.'

    $releaseDll = Join-Path $outputRoot 'RelayLiveLoop.ReleaseSurface.dll'
